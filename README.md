...
- 风机方案批准时在事务内绑定当前通风网络快照（`network_revisions` 单行版本号 + 全部启用节点/巷道的 SHA-256 内容指纹）；节点或巷道新增、停用、参数变化在同一事务推进版本号并把全部已批准方案立即回退到 `pending_review`、清空绑定并记录失效原因。
- 推演入口在锁定网络版本的事务内复核绑定快照：状态、版本号、内容指纹任一不符即返回 `409 SCENARIO_SNAPSHOT_STALE` 拒绝启动并提示重新复核；无变化可继续推演，复核员重新批准后绑定新快照恢复推演；历史推演的输入快照永久保留原网络。PostgreSQL 以 `FOR UPDATE` 行锁、SQLite 以单写连接串行化所有网络变更、批准与推演事务，并发变更不会让旧批准漏失效。
...
- 推演返回 `409 SCENARIO_SNAPSHOT_STALE`：批准后通风网络发生过节点/巷道新增、停用或参数变化，旧批准已自动回退到待复核；由复核员在方案页查看红色失效原因后重新批准（绑定最新网络快照）即可恢复，历史推演结果不受影响。
- 无实质字段变化的节点/巷道更新不会推进网络版本，已批准方案保持有效。
