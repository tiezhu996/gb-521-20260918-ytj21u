...
export function SimulationsPage() {
  ...
  const launch = async () => {
    ...
    try {
      ...
      message.success(`推演 #${run.id} 已完成并保存结果快照`);
    } catch (error) {
      reportError(error, '推演未能启动');
      // 网络变化导致批准失效时，刷新方案列表使复核员能看到重新复核提示。
      loadScenarios().catch((refreshError) => reportError(refreshError, '方案列表刷新失败'));
    } finally {
      setStarting(false);
    }
  };
  ...
  return (
    <div className="page">
      ...
      <Alert className="section-alert" type="info" showIcon message="仅当批准绑定的通风网络快照（版本号 + 内容指纹）与当前网络完全一致时推演入口才会启动；节点或巷道新增、停用、参数变化后旧批准立即失效，必须由复核员重新批准。" />
      <section className="run-launcher" aria-labelledby="launch-heading"><div><span className="section-index">01</span><h2 id="launch-heading">选择已批准方案</h2></div><Select aria-label="已批准方案" value={scenarioId} onChange={setScenarioId} options={approved.map((item) => ({ value: item.id, label: `${item.name} · v${item.version} · 网络快照 #${item.network_revision ?? '未绑定'}` }))} placeholder="当前没有快照一致的可运行方案" /><Button type="primary" icon={<Play size={17} />} disabled={!scenarioId || !hasRole('engineer', 'admin')} loading={starting} onClick={() => void launch()}>开始离线推演</Button></section>
